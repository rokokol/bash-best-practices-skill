...
**Every script says in its header which bash it needs.** An unstated floor is discovered only when the script lands on an older machine. The line is prose in the header comment, beside the other claims about the file, and not in the help, which describes the interface ([shape.md](shape.md))
