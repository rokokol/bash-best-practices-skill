...
`check-sh.sh` reads a script's dispatcher, its flag parsers, the variables it reads and the codes it exits with out of the source, and holds the help to them; with `-d DOC` it holds a document's `NAME sub` mentions to the dispatcher both ways, with `-c BASH ZSH` the two completion files, and with `-e PREFIX` the environment variables. A header claiming `Needs bash 3.2 and POSIX tools only` turns on a grep for newer constructs, labelled as the proxy it is; the proof is the [macos.yml](templates/github/workflows/macos.yml) workflow running the gate under the real `/bin/bash` 3.2. `check-sh.sh --template` prints the canonical script and its two completions, which are the text the checker plants its defects into on every run
