...
**Every script says in its header which bash it needs.** An unstated floor is discovered only when the script lands on an older machine. The line is prose in the header, so it is also in the text `--help` prints ([shape.md](shape.md))
