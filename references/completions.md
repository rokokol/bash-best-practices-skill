...
A completion is the third reader of a CLI, and the only one that answers while the command is still being typed. A tool earns a pair when a person types it by hand more than once, which in practice means a tool on PATH, invoked by its own name. The scripts inside this family's skills get none on purpose: `check-sh.sh`, `t.sh`, `ci.sh` and their kin are reached by path, by a gate or by an agent, never at a prompt, so a completion for them would be a list nobody presses TAB against and therefore a list nobody would notice going stale
...
bash-completion loads a file lazily by the command's name, and zsh searches `$fpath` for a file whose first line binds it to the command, so each is found by name with no line of user configuration anywhere — on NixOS `/etc/zshrc` walks `$NIX_PROFILES` and adds every profile's `site-functions` to `$fpath`, which is why installing the file is the whole job. The [huix-standard](https://github.com/rokokol/huix-standard-skill) installer copies both and records them in its manifest, so `--uninstall` takes them away again with everything else. The `.bash` extension exists only inside the repository, as the dialect marker shellcheck and shfmt read off the filename; installation drops it
...
- **`# shellcheck shell=bash` is the first line.** The file has no shebang to declare its dialect, and a shebang on a file that is only ever sourced would be a dead line. `check-sh.sh -c` reports a bash file that does not open with it: agents rolling the template out read it as redundant and dropped it
...
A value is static when it is known while the file is being written — subcommand names, flags, a fixed enumeration — and static values are spelled out and held to the dispatcher by machine. A value is dynamic when it is state the tool learns at runtime: profiles, installed effects, saved names. Dynamic values are produced by calling the tool at the moment TAB is pressed (`screen-shader menu 2>/dev/null | cut -d'|' -f2`), which puts two requirements back on the tool. The listing command must work without root, because `_sudo` restarts completion for the ordinary user before the privileged command ever runs, and it must print bare names on stdout with its noise on stderr, because the output goes straight into `compgen -W` or `_describe`
...
Only what a file offers is searched, not the prose around it. Comments are dropped from both files, and so is a case pattern that opens a line — `-l)`, `--prefix | --destdir)` — since that arm handles a flag's value and offers nothing: a flag dropped from the offered list survived there; and from the zsh file the text in `[...]` after an option, the description after the colon of a `'sub:description'` entry and the message between the first colons of an `'N:message:action'` spec — the action itself, `(run stop help)`, is what zsh offers and stays. Before that, the whole file was searched, so a subcommand removed from the list passed as long as another entry's description still said its name (found by the skill's own evaluation). The quotes are walked a line at a time, so a quoted string spanning lines is read as written

"Whole token" is the part that took a bug to get right. A substring match let `-f` pass on a file offering only `--force`, and `-h` on one offering only `--help`, so a missing short flag passed whenever its long twin was present; `grep -w` is wrong in the other direction, treating the hyphen as a word separator and so accepting `--help` inside `--help-all`. The matcher is `(^|[^[:alnum:]_-])FLAG([^[:alnum:]_-]|$)`, with the hyphen inside the token class on purpose — the rule was first written in huix-standard's `templates/tests/check-completions.sh:36-43`, which this flag replaces
