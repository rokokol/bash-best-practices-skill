...
- **`bash -n` runs first, on every file.** It parses without executing, so a missing `fi` is reported as a syntax error rather than as thirty shellcheck findings downstream of it — and under `tests/defects.sh` it is what turns a syntax-breaking edit into `unusable` instead of a defect the suite takes credit for catching (`tests/references/ecosystems/shell.md:80`)
...
A `# shellcheck disable=` comment is a last resort for a rule that is wrong about this code, never a way past a rule that is right (`huix-standard/references/install-sh.md:47`). The four that come up, each with its rewrite:
...
- **SC2016 — literal `${...}` in single quotes does not expand.** In a script that *generates* another script, not expanding is exactly the point, so write the generation as a heredoc with its `\$` escaped — shellcheck reads that cleanly, where a `printf` full of literal `${...}` needs a disable comment (`install-sh.md:27-28`)
...
**When the formatter's opinion changes between versions, the lock bump and the reformat land as one commit** (`install-sh.md:49`). shfmt 3.14 writes `((! x))` where 3.13 wrote `((!x))` — verified on this machine on 2026-09-11 with shfmt 3.14.0 — and each version rejects the other's spelling, so a bump without the reformat, or a reformat without the bump, leaves the weekly dependency cascade red over nothing the repository did. Split across two commits it is also unbisectable: every commit between them fails the gate for a reason unrelated to its own change

## One list of what gets linted

**The list of linted files lives once, in the gate's own `scripts=(…)`, and CI runs the gate rather than repeating the list** — `versioning/check.sh:15-17` carries both the list and the reason: "One source of truth for what gets linted. A second copy of this list drifts, and a drifted list lies about what was checked". Bash completions join it for shellcheck and `shfmt`; zsh ones go to a `zsh -n` loop beside it; and `check-sh.sh` is called from the same place, so a script cannot be linted without also being held to its own help. The general rule is the [ci](https://github.com/rokokol/ci-skill) skill's [one source of truth per list](https://github.com/rokokol/ci-skill/blob/main/references/checks.md#one-source-of-truth-per-list)

## Shell that writes shell

- **Every value pasted into a generated script goes through `sq`**, and it is not tidiness: the value and the path are executed by whatever runs the generated file, so pasted bare, a value holding a `"`, a `$(…)` or a backtick becomes code in it (`huix-standard/references/install-sh.md:30-41`). Single quotes are the one sh quoting with no expansion inside, so escaping the single quote itself is the whole job, and the result is checked against a value carrying all three and a prefix with a space in it
...
- **Every replacement spliced into a `sed` command is escaped first**, `sed 's/[&|\\]/\\&/g'` (`install-sh.md:51`): an unescaped `&` in the replacement expands to the whole match, and a path containing the delimiter ends the expression. The same class of bug as `sq`, in the tool that renders configs rather than scripts

What the scripts being linted must look like is [shape.md](shape.md), what their help must contain is [help.md](help.md), the version floors behind the rewrites above are in [portability.md](portability.md) and [pitfalls.md](pitfalls.md), the linter rules themselves are cited in [sources.md](sources.md), and the shell the agent's own ad-hoc commands run in — which is neither of the two dialects above — is [harness.md](harness.md)
