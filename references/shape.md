...
- **Every temporary lives under one `mktemp -d` with a template that names the owner, cleaned by a single `trap … EXIT`.** A fixed path under `/tmp` collides between two runs and is a symlink attack in a shared directory (`tests/references/ecosystems/shell.md:47`); `"${TMPDIR:-/tmp}/NAME.XXXXXX"` makes the leftovers of a crashed run greppable. What the template does and does not buy on a BSD `mktemp` is in [portability.md](portability.md)
...
**0 clean, 1 the thing asked about is wrong, 2 asked wrongly or there was nothing to check** — and "nothing to check" is a refusal, never a quiet pass, because an extractor that finds nothing must not read as "no drift" (`ci/references/checks.md:9`). A usage error is 2 in every script in this family, and the reason is a measurement rather than a taste: the survey of what the tools on this machine answer to an unknown flag is in [sources.md](sources.md#the-shape), and 2 is the only value with a plurality behind it

- **2 is what the family's checkers already answer** (`check-skill.sh:13`, `check-changelog.sh:21`); `ci.sh:188`, which exited 1 for an unknown subcommand, was the one place in the family that disagreed, and it was changed rather than copied
- **`"${2:?value required by $1}"` is not a usage guard** — it exits 1 with bash's own message, measured in [pitfalls.md](pitfalls.md#the-interpreter) — so the guard is `(($# >= 2)) || die "-n needs a name"` (`check-skill.sh:28-33`, `check-changelog.sh:38-41`, each with the reason on the line above)
