...
- **The delimiter is quoted wherever the help has nothing to substitute.** `<<'EOF'`, so `$1`, backticks and `${...}` in the examples stay literal — and shellcheck reads that cleanly, where a `printf` full of literal `${...}` needs a disable comment (SC2016, [lint.md](lint.md)). An unquoted `<<EOF` is the deliberate form for a help that prints a value the script holds — `install.sh — install NAME $VERSION`, which is what every installer in this family does — and it buys that substitution at the price of auditing every `$` and backtick in the text. `check-sh.sh` reports an unquoted delimiter whose body substitutes nothing at all, since that is the price paid for no purchase
