...
This is the shell-CLI case of the rule the [ci](https://github.com/rokokol/ci-skill) skill states for every list CI consults, [one source of truth per list](https://github.com/rokokol/ci-skill/blob/HEAD/references/checks.md#one-source-of-truth-per-list): hand-written mirrors allowed, drift-checked, in both directions. The tokens a completion must offer for the same flags are in [completions.md](completions.md)
