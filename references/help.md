...
- **A value-taking flag guards explicitly**, `(($# >= 2)) || die "-l needs a directory"`, never `"${2:?value required by $1}"`, which exits 1 with bash's own message where the header promised 2. `install-sh.md:17` prescribes `${2:?}` and exit 1 for an unknown flag; both are superseded here by the measurement in [shape.md](shape.md#exit-codes), and the huix template is being changed rather than followed
...
**A skill that ships a script says so in SKILL.md, in one line: "the help is the reference: run `X help [SUB]` before an unfamiliar command".** The point is not politeness to the reader but instruction to the next agent — a command guessed from documentation is a command nobody checked, and the documentation is a mirror by construction

This is the shell-CLI case of the rule the [ci](https://github.com/rokokol/ci-skill) skill states for every list CI consults, [one source of truth per list](https://github.com/rokokol/ci-skill/blob/main/references/checks.md#one-source-of-truth-per-list): hand-written mirrors allowed, drift-checked, in both directions. What the checker itself refuses to run on, and what it prints when it finds nothing to check, is in [shape.md](shape.md#exit-codes); the linting the same scripts are held to is in [lint.md](lint.md); the measured behaviours behind the extraction tricks are in [pitfalls.md](pitfalls.md) and [portability.md](portability.md), with citations in [sources.md](sources.md) and the harness's own quirks in [harness.md](harness.md)
