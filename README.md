...
`check-sh.sh` reads a script's dispatcher, its flag parsers, the variables it reads and the codes it exits with out of its syntax tree — `shfmt --to-json`, flattened by `jq` into one row per fact — and holds the help to them; with `-d DOC` it holds a document that lists the subcommands to the dispatcher both ways, with `-m DOC` only the mentions in a document that sends the reader to the help, with `-c BASH ZSH` the two completion files, and with `-e PREFIX` the environment variables. A header claiming `Needs bash X.Y` turns on a search for the constructs that arrived after that floor, and `POSIX tools only` one for the flags a BSD userland lacks — both labelled as the proxy they are, since a construct being present is not the script failing to run; the proof is the [macos.yml](templates/github/workflows/macos.yml) workflow running the gate under the real `/bin/bash` 3.2. Where neither tool is available, `-b, --bash-only` runs the checks that ask this bash a question and says in its summary that the rest did not run `check-sh.sh --template` prints the canonical script and its two completions, which are the text the checker plants its defects into on every run
